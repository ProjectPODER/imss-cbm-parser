let riesgoEmbarazoFull = {
    'A': "A. No hay riesgo para el feto en el primer trimestre del embarazo. No hay pruebas de riesgo en los últimos trimestres del embarazo.",
    'B': "B. Sin riesgo para el feto. Se carece de estudios clínicos adecuados para mujeres embarazadas.",
    'C': "C. Existen efectos secundarios en fetos de animales de experimentación. No hay hasta el momento estudios adecuados en los seres humanos, por lo que se desconocen los riesgos de su utilización en mujeres embarazadas. LA TERAPIA MEDICAMENTOSA SÓLO ES VALIDA CUANDO EL PROBLEMA DE SALUD INDICA SIN LUGAR A DUDAS, LA NECESIDAD DE SU EMPLEO.",
    'D': "D. Existen pruebas de riesgo para el feto humano. PUEDE ACEPTARSE EL RIESGO CUANDO LA PRESCRIPCIÓN INTRÍNSECA ES RACIONAL, A LO QUE SE AGREGA EN FORMA COALIGADA, UN PROBLEMA DE SALUD ESPECÍFICO E INDIVIDUAL.",
    'X': "X. Son evidentes las anormalidades en fetos humanos. Deben prevalecer los factores de riesgo, sobre los pretendidos “beneficios” del fármaco a prescribir. Contraindicado en el embarazo.",
    'NE': "NE. Se carece de estudios actuales que fundamenten factores de riesgo. Lo anterior hace considerar que EN CADA CASO, se tome en cuenta lo anotado en los puntos D o X."
}
...
function getGeneralidades(lines) {
    ...
    return {
        ...
        riesgo_embarazo: cleanString(getFullRiesgoString(riesgo_embarazo)),
...
function getFullRiesgoString(string) {
    if(string.length == 0) return '';

    let index = string.trim().split(' ')[0];
    if( riesgoEmbarazoFull.hasOwnProperty(index) ) {
        return riesgoEmbarazoFull[index];
    }
    else {
        return string;
    }
